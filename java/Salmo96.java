public class Salmo96 {

	public static void main(String[] args) {
		System.out.println("Cantai ao Senhor um cântico novo.");
		System.out.println("Cantai ao Senhor, todas as terras.");
		System.out.println("Cantai ao Senhor, bendizei o seu nome,");
		System.out.println("Proclamai a sua salvação.");
		System.out.println("Anunci'ai entre_as nações a sua glória;");
		System.out.println("Entre todos os povos, as suas maravilhas;");
		System.out.println("Porque grande_é o Senhor e mui digno de ser louvado,");
		System.out.println("Mais temível do que falsos deuses.");
		System.out.println("Porque_os outros deuses são ídolos apenas,");
		System.out.println("Porém os céus pelo Senhor foram feitos.");
		System.out.println("Glória_e majestade estão diante dele,");
		System.out.println("Força_e formosura no seu santuário.");
		System.out.println("");
		System.out.println("Rendei ao Senhor, famílias dos povos.");
		System.out.println("Rendei ao Senhor glória e força.");
		System.out.println("Rendei ao Senhor o_que_é devido_ao seu nome.");
		System.out.println("Entrai nos seus átrios com ofertas.");
		System.out.println("Adorai Su'a santidade porque é bela;");
		System.out.println("Tremei diante dele, povos de toda terra.");
		System.out.println("Dizei: “Reina o Senhor.” Firmou_o mundo p'ra que não se_abale.");
		System.out.println("Julga_os povos todos com_equidade.");
		System.out.println("Exultem terra_e céus; ruja_o mar e_o que há nele;");
		System.out.println("Regozije_o campo e su'a plenitude;");
		System.out.println("Di'ante do Senhor porque vem julgar a terra,");
		System.out.println("Julgará o mundo fiel_e justamente.");
	}
}